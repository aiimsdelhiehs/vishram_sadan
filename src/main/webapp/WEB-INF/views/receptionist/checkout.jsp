	        ...
	        <div class="card-body">
	          ...
	          <div class="row mb-2" ng-if="checkOutProceed">
	            <div class="col-lg-12 table-responsive">
	              <table class="table payment-receipt mt-3">
                    <tbody>
                        <tr><th colspan="9"><span ng-bind="currentBookingRequest.requestId"></span> &nbsp; Final Balance Receipt <span class="float-end" ng-bind="'Checkout On:'+checkoutdate"></span></th></tr>
                        <tr><th colspan="3">Sadan - Room & Category</th><th colspan="2">Guests</th><th colspan="2">Check-In</th><th colspan="2">Scheduled Check-Out</th><th>charges/day</th><th>days</th><th>Amount</th><th>Balance</th></tr>    
                        <tr ng-repeat="booking in checkOutBookings">
                            <td colspan="3">{{booking.sadan.name+' - '+floorIndex.get(booking.room.floor)+booking.room.name+' ('+booking.room.category.name+')'}}</td>
                            <td colspan="2">{{booking.attendants.length}}</td>
                            <td colspan="2">{{booking.fromDate.split('T')[0]}}</td>
                            <td colspan="2">{{booking.toDate.split('T')[0]}}</td>
                             <td>{{booking.room.category.price}}</td>
						...
						<div  class="col-lg-8" id="paymentReceipt">
							...
							<div >
								<div class="row" id="below">
									...
									<div class="col text"><span class="float-end" ng-bind="'दिनांक/Date: '+getFormattedDate(extendBookingOn)"></span></div>
								</div>
								<table class="table payment-receipt">
								 ...
								 <table class="table payment-receipt ">
                    <tbody>
                       <tr><th colspan="8"> &nbsp; Extend Booking Receipt <span class="float-end" ng-bind="'दिनांक/Extend On: '+getFormattedDate(extendBookingOn)"></span></th></tr>
                        <tr><th>Sadan - Room & Category</th><th>Guests</th><th>Check-In</th><th>New CheckOut</th><th>charges/day</th><th>days</th><th>Amount</th></tr>    
                        <tr ng-repeat="booking in extendsBookings">
                            <td>{{booking.sadan.name+' - '+floorIndex.get(booking.room.floor)+booking.room.name+' ('+booking.room.category.name+')'}}</td>
                            <td>{{booking.attendants.length}}</td>
                            <td>{{getFormattedDate(booking.fromDate)}}</td>
								...
								<div class="row justify-content-center">
									...
									<div class="col-4">
										<button class="btn btn-outline-success w-100" id="generatepdf"
											ng-click="generatePDF(extendData)">Generate PDF
										</button>
									</div>
								</div>
							</div>
