			...
			<div class="row justify-content-center ">
				<div class="col-lg-8" id="paymentReceipt">
					...
					<div class="row py-5" id="pdfHeader">
						...
						<div class="col-10">
							<center>
								<h3 class="font-weight-bold mb-0">विश्राम सदन, अंसारी नगर,
									नई दिल्ली-110029</h3>
								<h4 class="mb-0">VISHRAM SADAN, ANSARI NAGAR, NEW
									DELHI-110029</h4>
								<h5 class="mb-0">रसीद /RECEIPT</h5>
							</center>
						</div>
					</div>
					<div>
						<div class="row" id="below">
							<div class="col">
								<h5 class="text">
									रसीद सं./Receipt No: <b><i><span
											ng-if="(checkOutData.bookings[0].paymentHistory | filter:{receiptType:'Balance'}).length > 0">
												<!-- Get the txnId of the first 'BALANCE' payment -->
												{{(checkOutData.bookings[0].paymentHistory |
												filter:{receiptType:'Balance'})[0].txnId}}
										</span></i></b>
							...
							<div class="col text-end">
								<h5 class="text-end">
									बुकिंग संदर्भ संख्या/Booking reference no: <b><i>{{checkOutData.requestId}}</i></b>
								</h5>
							</div>
						</div>
						<div class="row">
							<div class="col">
								<h5 class="text">
									रसीद का प्रकार/Receipt Type:
									{{checkOutData.bookings[0].paymentHistory[0].receiptType}}</i></b>
								</h5>
							</div>
						...
						<table class="table payment-receipt">
							<tbody>
								...
								<tr>
									<th>नाम/Name</th>
									<td>{{checkOutData.patient.fullname}}</td>
									<th>UHID</th>
									<td class="text-end">{{checkOutData.patient.uhid}}</td>
									<th>Contact No.</th>
									<td>{{checkOutData.patient.contactNo}}</td>
									<th>पता/Address</th>
									<td class="text-end">{{checkOutData.patient.address}}</td>
								</tr>
							</tbody>
						</table>
						...
						<table class="table payment-receipt">
							<tbody>
								<tr>
									<th colspan="5">बुकिंग विवरण/Booking Description</th>
								</tr>

								<tr>
									<th colspan="12"><span
										ng-bind="currentBookingRequest.requestId"></span> &nbsp; Final
										Balance Receipt <span class="float-end"
										ng-bind="'Checkout On:'+today"></span></th>
								</tr>
								<tr>
								...
								<tr ng-repeat="booking in checkOutData.bookings">
									<td>{{booking.sadan.name+' -
										'+floorIndex.get(booking.room.floor)+booking.room.name+'
										('+booking.room.category.name+')'}}<br /> {{'Check In
										'+booking.fromDate.split('T')[0]}}<br /> {{'Check Out
									...
									<td colspan="6">
										<table>
											...
											<tr
												ng-show="payment.receiptType=='New_Booking'||payment.receiptType=='Re_New_Booking'"
												ng-repeat="payment in booking.paymentHistory">
												<td>{{payment.fromDate.split('T')[0]}}</td>
												<td>{{payment.toDate.split('T')[0]}}</td>
												<td>{{payment.rate+'*'+countDaysBetweenDates(payment.fromDate,payment.toDate)+'
													= '+payment.amount}}</td>
												<td>{{checkoutdate}}</td>
												...
												<td><b
													ng-class="(computeBalanceDays(payment.fromDate,payment.toDate,getDateString(checkoutdate))<0)?'text-danger':'text-success'">{{payment.rate*computeBalanceDays(payment.fromDate,payment.toDate,getDateString(checkoutdate))}}</b></td>
												</td>
											</tr>
                ...
                <td>{{ payment.amount }}</td>
                          ...
                          <td colspan="1">
											<b>Total Amount:</b></td>
											<td><b ng-bind="getTotalBookingAmount(checkOutData)+' Rs.'" ng-class="(getTotalBookingAmount(checkOutData)>=0)?'text-success':'text-danger'"></b></td>
                          <td colspan="6">
					      <b>
							    <span ng-if="getTotalBalanceAmount(checkOutData) >= 0">Due Amount:</span>
							    <span ng-if="getTotalBalanceAmount(checkOutData) < 0">Refund Amount:</span>
							  </b>
							</td>
                          <td><b ng-bind="getTotalBalanceAmount(checkOutData)+' Rs.'" ng-class="(getTotalBalanceAmount(checkOutData)>=0)?'text-success':'text-danger'"></b></td>
                        </tr>
						...
						<div class="row">
							<div class="col">
								<ul style="list-style-type: disc; padding-left: 20px;">
									...
									<li>Check-in time is 11:00 AM, and check-out time is 10:00
										AM.</li>
									<li>चेक-इन का समय सुबह 11:00 बजे है, और चेक-आउट का समय
										सुबह 10:00 बजे है।</li>
									<li>Please carry a valid ID during check-in.</li>
									<li>कृपया चेक-इन के दौरान एक वैध आईडी साथ रखें।</li>
									<li>If you check in and wish to check out within 1 hour,
										you are still required to pay for a full day.</li>
									<li>यदि आप चेक इन करते हैं और 1 घंटे के भीतर चेक आउट करना
										चाहते हैं, तो भी आपको पूरे दिन का भुगतान करना होगा।</li>
									<li>For rooms with a specified capacity, an additional
										charge of ₹20 per person per day will apply for each extra
										person beyond the room's standard capacity.</li>
									<li>निर्दिष्ट क्षमता वाले कमरों के लिए, कमरे की मानक
										क्षमता से परे प्रत्येक अतिरिक्त व्यक्ति के लिए प्रति दिन ₹20
										का अतिरिक्त शुल्क लागू होगा।</li>
								</ul>
							</div>
						</div>
						<div class="row">
						...
						<div class="row justify-content-center">
							...
							<div class="col-4">
								<button class="btn btn-outline-success w-100" id="generatepdf"
									ng-click="generatePDF(pdfData)">Generate PDF</button>
							</div>
						</div>
					</div>
				</div>
