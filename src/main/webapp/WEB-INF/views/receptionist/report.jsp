							...
							<table class="table payment-receipt">
								...
								<table class="table payment-receipt">
									<tbody>
										...
										<tr >
									      <td>{{receiptDetails.payments.booking.sadan.name }} - {{ receiptDetails.payments.booking.room.name }} & {{receiptDetails.payments.booking.room.category.name }}<br/>
									       {{'Check In '+payment.fromDate.split('T')[0]}}<br/>
                               				 {{'Check Out '+payment.toDate.split('T')[0]}}
									      </td>
									      <td>{{ payment.booking.attendants.length || 'N/A' }}</td> 
									      <!-- Replace 'N/A' with actual guest data if available -->
									        <td colspan="5">
                             <table>
                               <tr><td>From Date</td><td>To Date</td> <td>charges/day</td><td>days</td><td>Amount</td></tr>
                               <tr>
                                 <td>{{receiptDetails.fromDate.split('T')[0]}}</td>
                                 <td>{{receiptDetails.toDate.split('T')[0] }}</td>
                                 <td>{{receiptDetails.rate}}</td>
                                 <td>{{ countDaysBetweenDates(receiptDetails.fromDate, receiptDetails.toDate) }}</td>
